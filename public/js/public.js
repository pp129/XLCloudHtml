...
function rlFun(rldata) {
	...
	if (!rldata.isRadio) {
		$("#" + id + " .daysDiv").on("click", function () {
			...
			if(chooseTime === 2){
                ...
                for(var item of $(".daysDivActive")){
                    rldata.checkedDayArray.push($(".daysDiv").index($(item)) + 1);
                }
            }else{
                rldata.checkedDayArray = [$(".daysDiv").index(this) + 1];
            }
            if(rldata.onClickDay){
                rldata.onClickDay(rldata);
            }
		})
	} else {
		$("#" + id + " .daysDiv").on("click", function () {
	...
	$("#" + id + " .changeBtn").on("click", function () {
		...
		if(rldata.onChangeMonth){
            rldata.onChangeMonth(rldata);
        }
		rlFun(rldata);

	});
    rldata.reset = function (){
        $(".daysDivActive").removeClass("daysDivActive");
        rldata.checkedDayArray = [];
    }
    rldata.setData = function (data){
        var domArray = $(".daysDiv");
        for(var i = 0; i < data.length ; i++){
            $(domArray[i]).find(".daysText").text(data[i]);
        }
    }
}
