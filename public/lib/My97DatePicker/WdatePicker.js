/*
 * My97 DatePicker 4.72 Release License: http://www.my97.net/dp/license.asp
 */
var $dp, WdatePicker;
(function() {
	var _ = {
	...
	function N($) {
		$ = $ || V;
		var B = $[O], A = ($.innerWidth) ? $.innerWidth : (B[J] && B[J].clientWidth) ? B[J].clientWidth
		        : B.body.offsetWidth, _ = ($.innerHeight) ? $.innerHeight
		        : (B[J] && B[J].clientHeight) ? B[J].clientHeight : B.body.offsetHeight;
		return {
		    "width" : A,
		    "height" : _
		}
	...
	function U(J, C) {
		...
		if(C) {
			...
			if(!$dp.el || $dp.el["My97Mark"] === true || $dp.el.disabled
	...
	function K(H, $) {
		...
		if(!$dp.dd
			...
			if(_.$dpPath == "")
				F(A);
			var B = "<iframe style=\"width:1px;height:1px\" src=\"" + _.$dpPath
