...
rk.mapgis = function(options){
   ...
   * @type {基础图层瓦服务地址}
   */
   this.serviceURL = IsDefRK(options.serviceURL)?options.serviceURL: rk.DEFAULT.URL;
   this.serviceSateURL = IsDefRK(options.serviceSateURL)?options.serviceSateURL: rk.DEFAULT.URL;
   this.overlayURL = IsDefRK(options.overlayURL)?options.overlayURL: rk.DEFAULT.URL;
   /**
   * @private
   * @type {基础图层对象}
   */
   ...
   * @type {显示最小层级}
   */
   this.minZoom = IsDefRK(options.minZoom)?options.minZoom: rk.DEFAULT.MINZOOM;
   /**
   * @private
   * @type {显示最大层级}
   */
   this.maxZoom = IsDefRK(options.maxZoom)?options.maxZoom: rk.DEFAULT.MAXZOOM;
   /**
   * @private
   * @type {旋转角度}
   */
   this.rotation = IsDefRK(options.rotation)?options.rotation: rk.DEFAULT.ROTATION;
   /**
   * @private
   * @type {版权声明文本}
   */
   this.attributionText  = IsDefRK(options.attributionText)?options.attributionText: rk.DEFAULT.Attribution;
   /**
   * @private
   * @type {中心坐标}
   */
   ...
   * @type {瓦片坐标系}
   */
   this.projection =  IsDefRK(options.projection)?options.projection: rk.DEFAULT.PRO;
   /**
   * @private displayprojection = 'EPSG:4326'
   * @type {显示做坐标系}
   */
   this.displayprojection =  IsDefRK(options.displayprojection)?options.displayprojection: rk.DEFAULT.DISPLAYPRO;
   /*
   ** 鼠标事件 显示鼠标经过的经纬度
   */
   ...
   */
   ...
   this.CreatePopup = function(){
        $(document.getElementById(m_obj._target)).append('<div id="userpopup" class="ol-popup"><a href="#" id="userpopup_closer" class="ol-popup-closer"></a><div id="userpopup_content"></div></div>');
   ...
   this.SetFeatrueStyle =function(){
      $(m_obj._map.getViewport()).on('mousemove', function(e) {
		  ...
		  var hit = m_obj._map.forEachFeatureAtPixel(pixel, function(feature, layer) {
			return true;
		  });
		  if (hit) {
   ...
   */
   this.InitMapGis = function(){
		...
		m_obj._view = new ol.View({
			...
			center: m_obj.DisplayToUseTransForm([m_obj.center.lon, m_obj.center.lat]),
		...
		m_obj.baseLayer =  new ol.layer.Tile({
			  source: new ol.source.OSM({
				attributions: [
				  new ol.Attribution({html:m_obj.attributionText}),
		...
		m_obj._map =  new ol.Map({
		  ...
		  layers: [ m_obj.baseLayer,m_obj.baseSateLayer,m_obj.overlayLayer ],
		  view:m_obj._view,		
		  controls: ol.control.defaults({
			attributionOptions:({ collapsible: true})	
		  }).extend([
		    // new ol.control.Attribution({tipLabel:'声明'}),
		    //new ol.control.Zoom({zoomInTipLabel:"缩小",zoomOutTipLabel:"放大"}),
			//new ol.control.ScaleLine({ units: 'metric'}),
		    // new ol.control.ZoomSlider(),
			//new ol.control.OverviewMap({tipLabel:'缩略图', collapsed: true}),
...
rk.mapgis.prototype.SetMouseByClickCallbackFn = function(callbackFn){
	...
	this.mapclickkey = this._map.on('click', function(evt){
	   ...
	   try{
			myobj.mouseByClickCallbackFn({lon:coordinate[0],lat:coordinate[1]});
	   }catch(e){}
	});
};
/*
...
rk.mapgis.prototype.SetMouseByMoveCallbackFn = function(callbackFn){
	...
	this.mapmovekkey = this._map.on('pointermove', function(evt){
	   var coordinate = myobj._map.getEventCoordinate(evt.originalEvent);
	   coordinate = myobj.UseToDisplayTransForm(coordinate);
	   try{
			myobj.mouseByMoveCallbackFn({lon:coordinate[0],lat:coordinate[1]});
	   }catch(e){}
	});
};
//移动到地图中心
...
rk.mapgis.prototype.GetMapCenter = function(){
	var myobj = this;
	try{
		var center = myobj._view.getCenter(center) ;
...
//旋转反向
rk.mapgis.prototype.spiralToMadrid = function(lonlat, duration, showNum, roteteNum){
	var myobj = this;
	try{
		var bern = [lonlat.lon, lonlat.lat];
		bern = myobj.DisplayToUseTransForm(bern);//转换坐标
...
//启用选中
rk.mapgis.prototype.EnableSelectFeature = function(){
	var myobj = this;
